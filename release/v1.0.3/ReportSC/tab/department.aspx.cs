...
public partial class department : System.Web.UI.Page
{
    ...
    {
        DataTable dt = departmentSaleBLL.GetDepartmentSale(DateTime.Parse(txtBeginDate.Value), DateTime.Parse(txtEndDate.Value));
        rptProductSale.DataSource = dt;
        totalMoney = Math.Round(Global.Obj2Decimal(dt.Compute("sum(totalMoney)", "")), 2);
        supplyMoney = Math.Round(Global.Obj2Decimal(dt.Compute("sum(supplyMoney)", "")), 2);
